...
export async function PATCH(req: Request, { params }: { params: { eventId: string; sessionId: string } }) {
  const sb = makeClient()
...
export async function DELETE(_req: Request, { params }: { params: { eventId: string; sessionId: string } }) {
  const sb = makeClient()
  if (!await requireBoard(sb)) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })

  const { error } = await sb.from('event_sessions')
    .delete()
    .eq('id', params.sessionId)
    .eq('event_id', params.eventId)
